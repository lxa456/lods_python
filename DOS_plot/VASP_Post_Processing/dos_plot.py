...
# 本脚本适用于VASP计算流程的后处理。画出所有DFT文件夹中的DOS，并去除掉dft_data_exist.csv中的结构。
# step 1. prepare cmd.in 
# step 2. run vaspkit
# step 3. plot DOS + eigenval 
# step 4. save
#step 5. 把重复结构的DOS图放在另一个文件夹中。
...
if __name__=="__main__":
    ...
    #current_path = r"C:\Users\dell\Desktop\database_backups\B\DFT"
    ...
    # step 5. 把重复结构的DOS图move到另一个文件夹中。
    from shutil import move

    exist_data_csv = os.path.join(current_path, "DFT_data_screen","dft_data_exist.csv")
    filename_list = read_csv_first_column(exist_data_csv)
    print(filename_list)
    dos_exist = os.path.join(current_path, "DOS_image_existed")

    for root, dirs, files in os.walk(save_dir):
        for filename in files:
            if filename.endswith(".png") and filename.replace(".png", "") in filename_list:
                if not os.path.exists(dos_exist):
                    os.makedirs(dos_exist)      
                old_path = os.path.join(root, filename)
                new_path = os.path.join(dos_exist, filename)
